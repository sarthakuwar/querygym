...
else fail "HF Space /reset returned HTTP $HTTP_CODE"; stop_at "Step 1"; fi
log "${BOLD}Step 2/3: Running docker build${NC} ..."
if ! command -v docker &>/dev/null; then fail "docker command not found"; stop_at "Step 2"; fi
...
if [ "$VALIDATE_OK" = true ]; then pass "openenv validate passed"; [ -n "$VALIDATE_OUTPUT" ] && log "  $VALIDATE_OUTPUT"
